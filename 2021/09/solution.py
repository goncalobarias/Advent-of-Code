...
def find_surround_points(map, coord):
    ...
    points = list(
        filter(
            lambda x: 0 <= x[0] < len(map[0]) and 0 <= x[1] < len(map), points
        )
    )
...
def find_low_pp(data):
    ...
    for line in range(LD):
        for col in range(LL):
            ...
            bigger_points = list(
                filter(
                    lambda x: get_point_value(data, x)
                    > get_point_value(data, pp),
                    points,
                )
            )
    ...
    for line in range(LL):
        for col in range(CC):
            if get_point_value(map, (col, line)) == 9:
                ...
                for pp in points:
                    if get_point_value(map, pp) != 9 and pp not in blacklist:
                        point = pp
                try:
                    sizes += [check_points(map, point, blacklist)]
                except Exception:
                    pass
    sizes.sort()
