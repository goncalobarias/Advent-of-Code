def binary_decimal(bin_num):
    def aux(bin_num, value, position):
        if bin_num == '': return value

        return aux(bin_num[:len(bin_num) - 1], 
...
def part1():
    ...
    for position in range(len(data[0])):
        ...
        for number in range(len(data)):
            if data[number][position] == '0': zeros += 1
            else: ones += 1
    ...
    for position in range(len(data_processed[0])):
        if len(data_processed) > 1:
            ...
            for number in range(len(data_processed)):
                if data_processed[number][position] == '0': zeros += 1
                else: ones += 1
            if (zeros > ones and mode == 0) or (zeros <= ones and mode == 1):
                data_processed = list(filter(lambda x: x[position] == '0',
                                            data_processed))
            elif (zeros <= ones and mode == 0) or (zeros > ones and mode == 1):
                data_processed = list(filter(lambda x: x[position] == '1',
                                            data_processed))
        else: break
