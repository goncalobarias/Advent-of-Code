...
def find_numbers_part_2(data):
    ...
    for output in data:
        ...
        for dig in current_line:
            # Find digit 1
            if len(dig) == 2: numbers[1] += dig
            
            # Find digit 7
            elif len(dig) == 3: numbers[7] += dig

            # Find digit 4
            elif len(dig) == 4: numbers[4] += dig

            # Find digit 8
            elif len(dig) == 7: numbers[8] += dig

        # Viewing the list a second and last time to find the other digits.
        for dig in current_line:
            # Find digit 3
            if (len(dig) == 5 and numbers[1][0] in dig 
                 and numbers[1][1] in dig):
                numbers[3] += dig

            # Find digit 6
            elif (len(dig) == 6 
                  and ((numbers[1][0] in dig and numbers[1][1] not in dig) 
                  or (numbers[1][1] in dig
                  and numbers[1][0] not in dig))):
                numbers[6] += dig

            # Find digit 0
            elif (len(dig) == 6 
                 and len(list(l for l in dig if l not in numbers[4])) == 3): 
                numbers[0] += dig
            ...
            # Find digit 9
                  ...
                  and len(list(l for l in dig if l not in numbers[4])) == 2):
                numbers[9] += dig
